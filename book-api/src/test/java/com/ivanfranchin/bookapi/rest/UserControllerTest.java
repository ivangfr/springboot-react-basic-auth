...
class UserControllerTest {
  ...
  void getUsers_asAdmin_returns200() throws Exception {
    when(userService.getUsers())
        .thenReturn(List.of(newUser(1L, "admin", Role.ADMIN), newUser(2L, "user", Role.USER)));
  ...
  void getUser_unauthenticated_returns401() throws Exception {
    mockMvc.perform(get("/api/users/alice")).andExpect(status().isUnauthorized());
  }
  ...
  void deleteUser_asAdmin_returns204() throws Exception {
    ...
    mockMvc.perform(delete("/api/users/alice")).andExpect(status().isNoContent());
  }
  ...
  void deleteUser_selfDeletion_returns400() throws Exception {
    ...
    mockMvc.perform(delete("/api/users/admin")).andExpect(status().isBadRequest());
  }
  ...
  void deleteUser_lastAdmin_returns400() throws Exception {
    ...
    mockMvc.perform(delete("/api/users/alice")).andExpect(status().isBadRequest());
  }
